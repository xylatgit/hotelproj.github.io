...
Next, we investigated the proportion of cancellation with deposit type. The same process as above was applied to these two variables to generate the distribution and proportion plots. From the distribution bar plot, we found that majority of the deposit type for reservation is "no deposit" for both resort and city hotel. From the proportion plot, we found that among the canceled reservations, there are 94% and 99% of rooms reserved with non-refundable deposit had been canceled. This finding doesn't follow our expectations and intuitive, and therefore, we need to compare with other types of analysis for further investigation. Otherwise, for reservations with refundable deposit, we found that the cancellation of city hotel is relatively higher than that of resort hotel. 

Then, we investigated the proportion of cancellation with total number of special requests. The same process as above was applied to these two variables to generate the distribution and proportion plots. From the distribution bar plot, we found that most of the customers do not have special requests, or have only one special requests. Based on the proportion plot, we found that for both hotels, the cancellation proportion is higher when customers do not ask for special requests. 

We also investigated the proportion of cancellation with customer type. The same process as above was applied to these two variables to generate the distribution and proportion plots.From the distribution bar plot, we can see that majority of the customers the two hotels received were transient customers, and some of transient party customers. There are only a few group or contract customers. Based on the proportion plot, we found that for both city and resort hotel, the cancellation proportion for transient groups are significantly higher than other types of cutomers. It can be the reason that there are much more transient customers booking rooms at first. Additionally, there are about half of contract customers who reserve the city hotel would cancel the reservations. 

Lastly, we investigated the proportion of cancellation with market segment. The same process as above was applied to these two variables to generate the distribution and proportion plots. From the distribution bar plot, we found that most of the customers reserved the two hotels through online travel agent, and some of them reserved through offline travel agent or tour operators. And there are only a few reservations were made by corporate, complementary, and aviation segments. Based on the proportion plot, we found that the cancellation proportion is relatively higher among the groups, online travel agent, and offline travel agent or tour operator. 
